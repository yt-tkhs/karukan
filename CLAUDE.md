...
- Learning cache records user-selected conversions and boosts them on subsequent conversions; candidate priority: Learning → User Dictionary → Model → System Dictionary → Fallback → Rewriter. Surfaces longer than `max_surface_chars` (default 50, `[learning]` in config.toml; the learning limits travel together as `LearningConfig`) are not recorded. The automatic lists (composing suggestions, Space's mixed list) take their prefix-extending matches — the learning cache's (`LearningCache::predict`) and the dictionaries' (`predictive_search`) alike — only within `predict_extra_chars` (`[conversion]`) chars past the typed reading. The shipped default is 0: no candidate longer than what was typed, the user's explicit ask after two rounds of caps still let 「ほ」 offer 「本社」; at 4, 「ほん」 offers 本日 and 本当に, and a learned 「本日の日報です」 comes up only once the typing is within four kana of its end, however often it was committed (a frequency escape was tried and dropped: the sentence typed every morning was exactly the one that annoyed). Exact matches and the Ctrl+R learning / dictionary views (`prefix_lookup`, the uncapped browsers) ignore the cap. During conversion, Ctrl+Backspace or Ctrl+Delete (the Mac "delete" key is Backspace) removes the selected learning candidate from the history, mozc-style (`DeleteSelectedCandidate`): the removal clears the entry's whole prefix fan-out (`LearningCache::remove_suggestion`) so deduped twins under longer readings can't resurface, and the conversion is then rebuilt in place (mozc's cancel-and-reconvert, minus the window blink) so a surface that the model/dictionary/fallback also produce survives as an ordinary candidate. With a non-learning candidate selected the chord is consumed but does nothing (mozc's `DoNothing`); cancelling stays on plain Backspace/Escape; the chord exists only in the Conversion state (Composing keeps plain char editing). Deletability is derived from `Candidate::source` (`CandidateSource::is_deletable`, mozc's `USER_HISTORY_PREDICTION` analogue), and while a learning candidate is selected the aux text shows the mozc-style footer hint 「Ctrl+Backspaceで履歴から削除」
- Typing a printable character during a plain Conversion accepts the selected candidate and starts the next composition with that keystroke, as mozc does (`commit_and_continue`): the pick the user made (「サブステータス」) is committed as it stands, and the committed text is pushed onto the engine's left context (`push_left_context`) because the frontends refresh surrounding text only between compositions and this keystroke never leaves one. The search views refine instead: inside a narrowed source view or the emoji picker the keystroke drops back to the untouched composition and grows the reading, and the filter survives it (`refine_through_composing` re-enters the conversion with the same source; the intermediate composing render is discarded, so its auto-suggest inference is suppressed for that one call — the view runs its own lookup); plain Backspace mirrors that by shrinking the reading in place (unfiltered Backspace still cancels back to the composition). Ctrl+digit (1-9) selects a candidate — a bare digit is text like any other printable char (it commits the selection and starts the next composition with the digit), so typing numbers never conflicts with selection — Ctrl+J (`rebreak_conversion`) inserts a chunk break at the caret and rebuilds in place, narrowing what the beam covers while keeping the active source filter, and Enter commits
...
- Segments (文節): the Conversion state holds `segments: Vec<Segment>` (reading + its own `CandidateList` + its own source filter) and a `focus` index; `InputState::reading()` / `candidates()` / `filter()` are the focused segment's, so the source views, the aux and the history delete work per segment unchanged. Space opens one segment over the whole reading (today's list, predictive matches on); Shift+← / Shift+→ (`resize_focused_segment`) move the focused segment's end by one char, the char changing hands with the next segment (a new one when the focused is the last), and rebuild the segments from the focused one on (`rebuild_segments_from`): each converts after the text of those before it (`ConversionQuery::preceding` → `model_candidates`' lctx after the editor's), and a segment closed by a user-drawn boundary takes exact matches only (`Prediction::ExactOnly` → `LearningScope::Exact`, dictionary `predictive_limit` 0, in the mixed list and the source views alike) since a candidate whose reading ran past the boundary would double up with the next segment on commit; only the last segment stays `Extend`. Once split, ← → / Home / End / Ctrl+A・B・E・F move the focus (`move_focus`) instead of dropping to editing (Escape is the way back), Ctrl+J is inert, and Ctrl+digit picks and moves on to the next segment (mozc's select-and-advance), committing only on the last. `render_conversion` is the one place the conversion is drawn — `Preedit::from_segments` joins the selected texts, the focused one `Highlight` (macOS shows it as the selection within the marked text, the candidate window under it) with the caret at its end, the window showing the focused list — and `conversion_commit` joins every segment's selected text and yields one (reading, text) learning pair per segment, so a split reading teaches its parts separately. Shift+← while composing (`start_split_conversion`) is Space then Shift+←. User-facing doc: `docs/key-bindings.md`
