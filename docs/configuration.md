...
predict_extra_chars = 0         # 前方一致（補完）の候補が入力より長くてよい文字数（辞書・学習）。0 で補完なし
...
開発や設定の調整で内部の動きを確認したいときは、`Ctrl+Shift+V` で詳細表示に切り替えます。押した時点で表示が変わります（起動時から有効にするには `[display] verbose = true`）。このキーは補助テキストが画面に出ている間（入力中・変換中）だけ IME が受け取り、未入力時はアプリに渡ります。詳細表示では次の情報が加わります。
...
- 前方一致（補完）は既定では出ない（`[conversion] predict_extra_chars = 0`）。入力した読みの変換だけが並ぶ。値を 4 にすると、読みが入力より 4 文字以内だけ長いエントリが出る（「ほん」で「本日」は出るが「本日の日報です」は出ず、残り 4 文字以内まで打ってから出る）。辞書の前方一致も同じ設定に従う。読みを最後まで打った完全一致は設定に関わらず出る
