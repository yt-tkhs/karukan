...
predict_extra_chars = 4         # 前方一致の候補が入力より長くてよい文字数（辞書・学習）
...
- 前方一致で出るのは、読みが入力より4文字（`[conversion] predict_extra_chars`）以内だけ長いエントリ。「ほん」に対して「本日」は出るが「本日の日報です」は出ず、残り4文字以内まで打ってから出る（長文がその先頭の数文字を打つたびに先頭候補に出てくるのを防ぐ）。辞書の前方一致も同じ制限を受ける。読みを最後まで打った完全一致は長さに関わらず出る
